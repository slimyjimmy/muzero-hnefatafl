            ...
            for y0 in range(board_size):
                ...
                for dx, dy in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
                    for distance in range(1, max_distance + 1):
                        ...
                        action = (x0 * board_size + y0) * board_size**2 + (
                            x1 * board_size + y1
                        )
...
class Game(AbstractGame):
    ...
    def step(self, action):
        ...
        """
        observation, reward, done, game_result, player = self.env.step(action)
        return observation, reward * 20, done, game_result, player
